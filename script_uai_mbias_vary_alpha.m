...
for a=as
   ...
   % [~, confProbs] = estimateJointProbJT([1 3], jtalg, nVarsOr, domainCountsOr)
    ...
    for iter=1:nIters
        ...
        [auc(iter, 1, ia), sens(iter, 1, ia), spec(iter, 1, ia), probs_1(:, ia, iter)]= estimateMetrics(bestSet, bestprobs, configs, testData(1:1000, :));
...
legend({'FIMB', 'FGESMB($D_e$)', 'FGESMB($D_o$)'}, 'Interpreter', 'latex', 'Location', 'NorthWest')
