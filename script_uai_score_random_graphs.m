...
for iter=11:50
    ...
    for iDoN =1:nDoNs
        ...
        [auc(iter,1, iDoN), sens(iter, 1, iDoN),  spec(iter, 1, iDoN), cmbProbs(:, iter, iDoN)]= estimateMetrics(bestSet, bestprobs, configs, testData);
...
for iDoN=1:nDoNs
    ...
    xticklabels({'FindIMB', 'IMB', 'OMB', 'FCIt-IMB'})
    ...
    [pvals(1) ]=signrank(biasFIMB, biasFGESIMB);
    [pvals(2) ]=signrank(biasFIMB, biasFGESMB);
    [pvals(3) ]=signrank(biasFIMB, biasFCIc);
    sigstar({[1 2], [1 3], [1 4]}, pvals);

%    end


    saveas(gcf, ['C:/Users/sot16.PITT/Dropbox/Apps/Overleaf/Causal Markov Blankets/figures/' figName], 'pdf')
